...
//            e.printStackTrace();
    ...
    public <T> boolean add(T model) {
        ...
        if (id == null) {
            response = client.prepareIndex(indexName, typeName).setSource(json, XContentType.JSON).execute().actionGet();
        } else {
            //6.3版本
            response = client.prepareIndex(indexName, typeName, id).setSource(json, XContentType.JSON).get();
//            response = client.prepareIndex(indexName, typeName, id).setSource(json,XContentType.JSON).execute().actionGet();
            ...
            for (T model : models) {
                try {
                    ...
                    if (id == null) {
                        ...
                        bulkRequest.add(client.prepareIndex(indexName, typeName, id).setSource(json, XContentType.JSON));
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    continue;
    ...
    public <T> Result searchFun(Class<T> clazz, BooleanCondtionBuilder params) {
        ...
        Map<String, String> strHighLightList = params.getHighLightFileds();
        if (strHighLightList != null && strHighLightList.size() > 0) {
            for (Map.Entry<String, String> entry : strHighLightList.entrySet()) {
                highlightBuilder.field(entry.getValue());
        ...
        for (SearchHit hit : hits.getHits()) {
            ...
            T model = null;
            if (resultMap != null) {
                model = SearchUtil.MapToModelParams(resultMap, clazz, hit,params.getHighLightFileds());
            }
            list.add(model);
        }
