...
var line_single_option = {
	...
	/*
	 * toolbox: { feature: { magicType: {show: true, type: ['line', 'bar']},
...
function get_mony_line_option_ajax(url, echartsobj, options, id) {
	...
	$.ajax({
		...
		success : function(data, status) {
			if (data.success == true || data.success == 'true') {
				...
				set_line_total_statistics(data.data, id);
				...
				for (var i = 0; i < seriesList.length; i++) {
					...
					/*
					 * //系列中的数据标注内容 ,markPoint: { data: [ {type: 'max', name:
					 * '最大值'}, {type: 'min', name: '最小值'} ] }, //系列中的数据标线内容
...
function load_mony_line(url, id, title, subtext, yAxis) {
	...
	echartsobj.showLoading();
	get_mony_line_option_ajax(url, echartsobj, mony_line_option, id);
	return echartsobj;
}
...
function load_mony_line_down(url, id, title, subtext, yAxis, color) {
	...
	echartsobj.showLoading();
	get_mony_line_option_ajax(url, echartsobj, mony_line_option_02, id);
	return echartsobj;
}
...
function get_mony_line_option_ajax_dt(url, echartsobj, options, id) {
...
function load_mony_line_dt(url, id, title, subtext, yAxis) {
	...
	echartsobj.showLoading();
	get_mony_line_option_ajax_dt(url, echartsobj, option_dt, id);
	return echartsobj;
}
...
// 多柱图 -统计
function set_line_total_statistics(data, id) {
	// 专利
	if (id == 'institute_contract_line') {
		var fyxRate = getDataCountForNameFloat(data, '费用性');
		var zbxRate = getDataCountForNameFloat(data, '资本性');
		if (typeof (data.seriesList) == "undefined") {

		} else {
			fyxRate = fyxRate / data.seriesList[0].data.length;
			zbxRate = zbxRate / data.seriesList[0].data.length;
		}

		$("#institute_contract_line_02").html(fyxRate.toFixed(2) + "%");
		$("#institute_contract_line_03").html(zbxRate.toFixed(2) + "%");
	}
