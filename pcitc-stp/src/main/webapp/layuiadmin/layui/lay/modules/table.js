...
;
layui.use(['jquery'], function () {  //如果只加载一个模块，可以不填数组。如：layui.use('form')
    var $ = layui.jquery;//获取jq
    window.commonDislodgeTable = function () {
        ...
        var _treeTableContainer = $('.treeTableContainer').length;
        var _treeTableBox = $('.treeTableBox').length;
        var _twoTablesContainer = $('.twoTables-container').length;
        var _tool = $('.tableToolBox').outerHeight(true) ? $('.tableToolBox').outerHeight(true) : 0;
        var _layuiTab = $('.layui-tab-title').outerHeight(true) ? $('.layui-tab-title').outerHeight(true) + 10 : 0;
        var _bottom = $('.layui-footer').outerHeight(true) ? $('.layui-footer').outerHeight(true) : 0;
        var tableHeight;
        if (_treeTableContainer) {
            if (_search > 0) {
                if (_treeTableBox > 0) {
                    _treeTableContainer = 10;
                } else {
                    _treeTableContainer = 0;
                }
            } else {
                _treeTableContainer = 10;
            }
        ...
        if (_twoTablesContainer > 0) {
            tableHeight = (_window - _top - _pagetabs - _search - _treeTableContainer - _tool - _layuiTab - _bottom - 56) / 2;
        } else {
            tableHeight = _window - _top - _pagetabs - _search - _treeTableContainer - _tool - _layuiTab - _bottom - 10;
        }
        /*console.log("_window:"+_window)
        console.log("_top:"+_top)
        console.log("_nav:"+_pagetabs)
        console.log("_search:"+_search)
        console.log("_treeTableContainer:"+_treeTableContainer)
        console.log("_twoTablesContainer:"+_twoTablesContainer)
        console.log("_treeTableBox:"+_treeTableBox)
        console.log("_tool:"+_tool)
        console.log("_layuiTab:"+_layuiTab)
        console.log("tableHeight:"+tableHeight)*/
        return tableHeight < 134 ? 134 : tableHeight;
    };
...
layui.define(["laytpl", "laypage", "layer", "form", "util"], function (e) {
    ...
    var t = layui.$, i = layui.laytpl, a = layui.laypage, l = layui.layer, n = layui.form,
        ...
        }, h = "table", y = ".layui-table", f = "layui-hide", p = "layui-none", v = "layui-table-view",
        ...
        H = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">', "{{# if(d.data.toolbar){ }}", '<div class="layui-table-tool">', '<div class="layui-table-tool-temp"></div>', '<div class="layui-table-tool-self"></div>', "</div>", "{{# } }}", '<div class="layui-table-box">', "{{# if(d.data.loading){ }}", '<div class="layui-table-init" style="background-color: #fff;">', '<i class="layui-icon layui-icon-loading layui-icon"></i>', "</div>", "{{# } }}", "{{# var left, right; }}", '<div class="layui-table-header">', E(), "</div>", '<div class="layui-table-body layui-table-main">', z, "</div>", "{{# if(left){ }}", '<div class="layui-table-fixed layui-table-fixed-l">', '<div class="layui-table-header">', E({fixed: !0}), "</div>", '<div class="layui-table-body">', z, "</div>", "</div>", "{{# }; }}", "{{# if(right){ }}", '<div class="layui-table-fixed layui-table-fixed-r">', '<div class="layui-table-header">', E({fixed: "right"}), '<div class="layui-table-mend"></div>', "</div>", '<div class="layui-table-body">', z, "</div>", "</div>", "{{# }; }}", "</div>", "{{# if(d.data.totalRow){ }}", '<div class="layui-table-total">', '<table cellspacing="0" cellpadding="0" border="0" class="layui-table" ', '{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>', '<tbody><tr><td><div class="layui-table-cell" style="visibility: hidden;">Total</div></td></tr></tbody>', "</table>", "</div>", "{{# } }}", "{{# if(d.data.page){ }}", '<div class="layui-table-page">', '<div id="layui-table-page{{d.index}}"></div>', "</div>", "{{# } }}", "<style>", "{{# layui.each(d.data.cols, function(i1, item1){", "layui.each(item1, function(i2, item2){ }}", ".laytable-cell-{{d.index}}-{{i1}}-{{i2}}{ ", "{{# if(item2.width){ }}", "width: {{item2.width}}px;", "{{# } }}", " }", "{{# });", "}); }}", "</style>", "</div>"].join(""),
        R = t(window), j = t(document), F = function (e) {
            var i = this;
    ...
    F.prototype.config = {
        ...
        var e = this;
        e.fullSize(), e.setColsWidth(), e.scrollPatch(), e.layNone && e.layNone.width(e.elem.width() - 2)
    }, F.prototype.reload = function (e) {
        var i = this;
        e = e || {}, e.data && e.data.constructor === Array && delete i.config.data, i.config = t.extend(!0, {}, i.config, e), i.render()
        ...
        if (i.startTime = (new Date).getTime(), a.url) {
            ...
            a.contentType && 0 == a.contentType.indexOf("application/json") && (d = JSON.stringify(d)), i.loading(), t.ajax({
                ...
                headers: a.headers || {},
                success: function (t) {
                    "function" == typeof a.parseData && (t = a.parseData(t) || t),t.code=="401" ? i.loginC() : "", t[n.statusName] != n.statusCode ? (i.renderForm(), i.errorView(t[n.msgName] || '返回的数据不符合规范，正确的成功状态码应为："' + n.statusName + '": ' + n.statusCode)) : (i.renderData(t, e, t[n.countName]), o(), a.time = (new Date).getTime() - i.startTime + " ms"), i.setColsWidth(), "function" == typeof a.done && a.done(t, e, t[n.countName])
                },
                error: function (e, t) {
            ...
            c[n.dataName] = a.data.concat().splice(s, a.limit), c[n.countName] = a.data.length, i.renderData(c, e, a.page && a.page.count ? a.page.count : a.data.length), o(), i.setColsWidth(), "function" == typeof a.done && a.done(c, e, c[n.countName])
        }
    }, F.prototype.eachCols = function (e) {
        var t = this;
        return d.eachCols(null, e, t.config.cols), t
    }, F.prototype.renderData = function (e, n, o, r) {
        var c = this, s = c.config, h = e[s.response.dataName] || [], y = [], p = [], v = [], m = function () {
            var e;
            return !r && c.sortKey ? c.sort(c.sortKey.field, c.sortKey.sort, !0) : (layui.each(h, function (a, l) {
                var o = [], h = [], m = [], g = a + s.limit * (n - 1) + 1;
                0 !== l.length && (r || (l[d.config.indexName] = a), c.eachCols(function (n, r) {
                    var c = r.field || n, y = s.index + "-" + r.key, p = l[c];
                    if (void 0 !== p && null !== p || (p = ""), !r.colGroup) {
                        var v = ['<td data-field="' + c + '" data-key="' + y + '" ' + function () {
                            var e = [];
                            ...
                            return "normal" === r.type ? y : y + " laytable-cell-" + r.type
                        }() + '">' + function () {
                            ...
                            return r.toolbar ? i(t(r.toolbar).html() || "").render(n) : u(r, p, n)
                        }(), "</div></td>"].join("");
                        o.push(v), r.fixed && "right" !== r.fixed && h.push(v), "right" === r.fixed && m.push(v)
                    }
                }), y.push('<tr data-index="' + a + '">' + o.join("") + "</tr>"), p.push('<tr data-index="' + a + '">' + h.join("") + "</tr>"), v.push('<tr data-index="' + a + '">' + m.join("") + "</tr>"))
            }), c.layBody.scrollTop(0), c.layMain.find("tbody").html(y.join("")), c.layFixLeft.find("tbody").html(p.join("")), c.layFixRight.find("tbody").html(v.join("")), c.renderForm(), "number" == typeof e && c.setThisRowChecked(e), c.syncCheckAll(), c.haveInit ? c.scrollPatch() : setTimeout(function () {
                c.scrollPatch()
            }, 50), c.haveInit = !0, l.close(c.tipsIndex), s.HAS_SET_COLS_PATCH || c.setColsPatch(), void(s.HAS_SET_COLS_PATCH = !0))
        };
        return d.cache[c.key] = h, c.layPage[0 == o || 0 === h.length && 1 == n ? "addClass" : "removeClass"](f), r ? m() : 0 === h.length ? (c.renderForm(), c.errorView(s.text.none)) : (m(), c.renderTotal(h), void(s.page && (s.page = t.extend({
        ...
        var n, r, c = this, s = {}, u = c.config, y = u.elem.attr("lay-filter"), f = d.cache[c.key];
        ...
        c.sortKey = {
            field: n,
            sort: i
        }, u.autoSort && ("asc" === i ? r = layui.sort(f, n) : "desc" === i ? r = layui.sort(f, n, !0) : (r = layui.sort(f, d.config.indexName), delete c.sortKey)), s[u.response.dataName] = r || f, c.renderData(s, c.page, c.count, !0), l && layui.event.call(e, h, "sort(" + y + ")", {
            field: n,
            type: i
        })
    }, F.prototype.loading = function (e) {
        var i = this, a = i.config;
        a.loading && (e ? (i.layInit && i.layInit.remove(), delete i.layInit, i.layBox.find(b).remove()) : (i.layInit = t(['<div class="layui-table-init">', '<i class="layui-icon layui-icon-loading layui-icon"></i>', "</div>"].join("")), i.layBox.append(i.layInit)))
        ...
        var e, a = this, o = a.config, c = t("body"), s = {}, u = a.layHeader.find("th"), y = ".layui-table-cell",
        ...
        a.layTool.on("click", "*[lay-event]", function (e) {
            ...
            switch (layui.stope(e), j.trigger("table.tool.panel.remove"), l.close(a.tipsIndex), c) {
                case"LAYTABLE_COLS":
                    s({
                        list: function () {
                            var e = [];
            ...
            layui.event.call(this, h, "toolbar(" + p + ")", t.extend({event: c, config: o}, {}))
        ...
        a.elem.on("click", 'input[name="layTableCheckbox"]+', function () {
            ...
            }), a.syncCheckAll(), a.renderForm("checkbox")) : (a.setCheckData(l, n), a.syncCheckAll()), layui.event.call(e[0], h, "checkbox(" + p + ")", v.call(e[0], {
                checked: n,
                type: o ? "all" : "one"
            }))
            ...
            }), a.setThisRowChecked(n), layui.event.call(this, h, "radio(" + p + ")", v.call(this, {checked: i}))
        ...
        var m = function (e) {
            var i = t(this);
            i.data("off") || layui.event.call(this, h, e + "(" + p + ")", v.call(i.children("td")[0]))
        };
        a.layBody.on("change", "." + W, function () {
            var e = t(this), i = this.value, l = e.parent().data("field"), n = e.parents("tr").eq(0).data("index"),
                o = d.cache[a.key][n];
            o[l] = i, layui.event.call(this, h, "edit(" + p + ")", v.call(this, {value: i, field: l}))
        }).on("blur", "." + W, function () {
            ...
            var i = t(this), a = (i.data("field"), i.data("edit")), l = i.children(y);
            if (!i.data("off") && a) {
        ...
        a.layBody.on("click", "." + g, function (e) {
            var i = t(this), n = i.parent(), d = n.children(y);
            ...
            layui.event.call(this, h, "tool(" + p + ")", v.call(this, {event: e.attr("lay-event")})), a.setThisRowChecked(i)
        ...
        i = i || {};
        var a = this, l = t(e ? 'table[lay-filter="' + e + '"]' : y + "[lay-data]"),
