layui.define(['jquery', 'laytpl', 'layer'], function (e) {
    ...
    job.prototype.pullData = function () {
        ...
        (!_config.cache && _config.ajax[_self.index] != undefined) && _config.ajax[_self.index].abort(), _config.ajax[_self.index] = $.ajax({
            ...
            url: _config.url,
            data: _config.data||{keywords: _config.filter},
            dataType: "json",
            beforeSend: function () {
                var fun = (eval("autocompleteCallBack"));
                if (fun) {
                    this.data = fun.call(this, this.data, this.keyword);
                    this.url = this.url +fun.call(this, this.data, this.keyword);
                }
                // console.log(this.url);
                // this.data={modelName:$('#modelName').val(),limit:"10",offset:"0"};
    ...
    job.prototype.events = function () {
        ...
        _elem.on('focus', function () {
            console.log("focus:修改为change的function");
