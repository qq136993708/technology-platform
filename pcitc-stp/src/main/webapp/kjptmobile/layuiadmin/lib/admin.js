...
layui.define('view', function(exports){
  ...
  ,admin = {
    ...
    ,theme: function(options){
      ...
      ,styleText = laytpl([
        ...
        ,'{background-color:{{d.color.selected}};}'
    ...
    ,tabsBody: function(index){
      return $(APP_BODY).find('.'+ TABS_BODY).eq((index+1) || 1);
    }
  ...
  $body.on('click', '*[lay-href]', function(){
    ...
    ,functionbuttons=othis.attr('data-functionbuttons')
    ,router = layui.router();
      code=othis.attr('data-code')
    admin.tabsPage.elem = othis;
    //admin.prevRouter[router.path[0]] = router.href; //记录上一次各菜单的路由信息
    //console.log(othis)
    //执行跳转
    var topLayui = parent === self ? layui : top.layui;
    topLayui.index.openTabsPage(href, text,id,code,functionbuttons || othis.text());
    $(".index-fixed li").removeClass("layui-this");
    /*setTimeout(function(){
        //高度自适应
        var topBottomSumHeight,
            topHeight,
            bottomHeight = 36;
        var searchBox =  document.getElementById(code).contentWindow.document.getElementsByClassName('searchBox')
        var layuiTabTitle = document.getElementById(code).contentWindow.document.getElementsByClassName('layui-tab-title')
        if(searchBox.length>0){
            topHeight = 58;
        }else if (layuiTabTitle.length>0){
            topHeight = 58;
        }else{
            topHeight = 28;
        }
        topBottomSumHeight = topHeight + bottomHeight;
        window.localStorage.setItem("topBottomSumHeight",topBottomSumHeight);
