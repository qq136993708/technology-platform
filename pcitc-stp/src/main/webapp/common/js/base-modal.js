...
(function ($, window, document, undefined) {
    ...
    function _modal_structure(config, ok, cancel) {
        ...
        modal.setAttribute('name', config.id);
        ...
        if (ok || cancel || config.has_footer) {
            ...
            footer.className = 'box-footer text-right bf-width';
    ...
    function _create_modal(config, ok, cancel) {
        ...
        parent.$('#modal-tips-div').modal('show');

        parent.$('#modal-tips-div').on("hidden.bs.modal", function() {
            modals.fixwyhtml5();
    	   _remove_modal();
    ...
    function _remove_modal() {
        ...
        parent.$("div.modal-backdrop").remove();
    }
    ...
    modals.openWin=function(config){
        ...
        parent.$("#"+winId).modal({remote:config.url,backdrop:config.backdrop||_win_config.backdrop,keyboard:config.keyboard||_win_config.keyboard});
        console.log(parent.$("#"+winId))
    ...
    modals._create_modal=function(config){
         ...
         modal.tabindex = -1;
         modal.role = 'dialog';
         modal.name=config.id;
         modal.setAttribute('aria-labelledby', 'myModalLabel');
         modal.setAttribute('aria-hidden',true);

        modal.setAttribute('param', config.param);
