...
public class ImageUtils {
	...
	// * @param h int 新高度
	// */
	// public void compressImgToSize(int w, int h, String newfileurl) throws
	// IOException {
	...
	// WBMP, GIF, gif]
	// String types =
	// Arrays.toString(ImageIO.getReaderFormatNames()).replace("]", ",");
	// String suffix = null;
	// // 获取图片后缀
	// if (srcImg.getName().indexOf(".") > -1) {
	// suffix = srcImg.getName().substring(srcImg.getName().lastIndexOf(".") +
	// 1);
	 ...
	 * @param newFilePath
	 */
	public static String getImgSize(int width, int height, InputStream inputStream, String newFilePath) {
		/*
		...
		String ossPath = "";
		try {
			Thumbnails.of(inputStream).size(width, height).outputQuality(1f).toFile(newFilePath);
			File temFile = new File(newFilePath);
			ossPath = OSSUtil.uploadFileWithFilePath(temFile, newFilePath);
		} catch (IOException e) {
			e.printStackTrace();
		}
		return ossPath;
	 ...
	 * @param newFilePath
	 */
	public static String getImgSizeNoScale(int width, int height, InputStream inputStream, String newFilePath) {
		String ossPath = "";
		try {
			Thumbnails.of(inputStream).size(width, height).keepAspectRatio(false).toFile(newFilePath);
			File temFile = new File(newFilePath);
			ossPath = OSSUtil.uploadFileWithFilePath(temFile, newFilePath);
		} catch (IOException e) {
			e.printStackTrace();
	 ...
	 * @param newFilePath
	 */
	public static String getImgScale(double scale, InputStream inputStream, String newFilePath) {
		String ossPath = "";
		try {
			Thumbnails.of(inputStream).scale(scale).outputQuality(1f).toFile(newFilePath);
			File temFile = new File(newFilePath);
			ossPath = OSSUtil.uploadFileWithFilePath(temFile, newFilePath);
		} catch (IOException e) {
			e.printStackTrace();
	...
	public static void getImgRotate(int width, int height, String filePath, String newFilePath, int rotate) {
		try {
			Thumbnails.of(filePath).size(width, height).rotate(rotate).toFile(newFilePath);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public static void getImgWatermark(int width, int height, String filePath, String newFilePath, String waterMarkPath, float waterMarkSize, float outputQuality) {
		try {
			Thumbnails.of(filePath).size(width, height).watermark(Positions.BOTTOM_RIGHT, ImageIO.read(new File(waterMarkPath)), waterMarkSize).outputQuality(outputQuality).toFile(newFilePath);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public static void batImagegetCompose(String path, String newpath, String modelPic, String modelPicName) {
		...
		for (int i = 0; i<tempList.length; i++) {
			...
			if (FileUtil.isImage(fileName)) {
				...
				// getImgSize(725,725,fileName,newpath + File.separator +
				// "750_"+tempList[i]);
				// getImgSize(750,750,fileName,newpath + File.separator +
				// "750_"+tempList[i]);
				// getImgSize(325,325,fileName,newpath + File.separator +
				// "325_"+tempList[i]);
				// getImgSize(300,300,fileName,newpath + File.separator +
				// "325_"+tempList[i]);
			}
	...
	public static void main(String[] args) {
		...
		// String strback =
		// "D:\\\\files\\\\uploadPath\\\\file\\\\getImgSize.jpg";
