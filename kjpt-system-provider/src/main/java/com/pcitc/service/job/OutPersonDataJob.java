	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			response = RestfulHttpClient.getClient(Quality)
					...
					.request();
			//保存任务日志
			QrtzUtils.saveSysQrtzLog(context, sysJobService, "1", "com.pcitc.service.job.OutPersonDataJob", response.getContent());
			
		} catch (IOException e) {
			e.printStackTrace();
			//保存任务日志
			QrtzUtils.saveSysQrtzLog(context, sysJobService, "2", "com.pcitc.service.job.OutPersonDataJob", e.getMessage());
