...
public class HomeServiceImpl implements IHomeService{
    ...
    {
    	List<H1AMKYSY100117> one = homePageMapper.get_home_KYZB(map);
    	return one;
    }
    public  List<H1AMKYSY100117> get_home_KYZB_02(Map map)throws Exception
    {
    	List<H1AMKYSY100117> one = homePageMapper.get_home_KYZB_02(map);
    	return one;
    }
    ...
    public LayuiTableData get_home_KYZB_detail(LayuiTableParam param)throws Exception
    {
		...
		int start=(pageNum-1)*p.getPageSize();
  		...
  		String begin_money=getTableParam(param,"begin_money","");
  		String end_money=getTableParam(param,"end_money","");
  		int beginMoney=0;
  		int endMoney=0;
  		if(!begin_money.equals(""))
  		{
  			beginMoney=Integer.valueOf(begin_money).intValue()*10000;
  		}
  		if(!end_money.equals(""))
  		{
  			endMoney=Integer.valueOf(end_money).intValue()*10000;
  		}
  		
  		logger.info("=====表格 param: "+JSONObject.toJSONString(param));
  		
  		Map map=new HashMap();
  		map.put("begin_money", beginMoney);
  		map.put("end_money", endMoney);
  		map.put("start", start);
  		map.put("pageSize", pageSize);
  		map.put("month", month);
  		map.put("type", type);
