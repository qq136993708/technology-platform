...
public class ZjkBaseInfoController extends BaseController {
	 ...
	 * 更新审核状态
	 */
	private static final String updateAuditStatus = "http://kjpt-zuul/stp-proxy/zjkbaseinfo-provider/zjkbaseinfo/updateAuditStatus/";
	/**
	 * 根据类型更新专家信息
	 */
	private static final String updateExpertByType = "http://kjpt-zuul/stp-proxy/zjkbaseinfo-provider/zjkbaseinfo/updateExpertByType";
	/**
	 * 物理删除
	 */
	...
	private static final String LISTPAGECount = "http://kjpt-zuul/stp-proxy/zjkbaseinfo-provider/zjkbaseinfo/zjkbaseinfo-page-count";
	private static final String showExpertPageTableData = "http://kjpt-zuul/stp-proxy/zjkbaseinfo-provider/zjkbaseinfo/showExpertPageTableData";
	/**
	 * 保存
	 */
	 ...
	 * 院士
	 */
	private static final String ZJK_YS_LIST = "http://kjpt-zuul/stp-proxy/zjkbaseinfo-provider/zjkbaseinfo/zjkYsList";

	private static final String START_WORKFLOW_URL = "http://kjpt-zuul/system-proxy/workflow-provider/workflow/start";
	...
	public Result startFlow2(@RequestBody String param, HttpServletRequest request) {
		...
		// 流程完全审批通过时，调用的方法
		variables.put("auditAgreeMethod", "http://kjpt-zuul/stp-proxy/zjkbaseinfo-provider/zjkbaseinfo/updateAuditStatus?dataId=" + "agree_" + businessId);

		// 流程驳回时，调用的方法（可能驳回到第一步，也可能驳回到第1+n步
		variables.put("auditRejectMethod", "http://kjpt-zuul/stp-proxy/zjkbaseinfo-provider/zjkbaseinfo/updateAuditStatus?dataId=" + "reject_" + businessId);
	...
	public Object getzjkBaseInfoInfo(HttpServletRequest request) {
		...
		// ResponseEntity<LayuiTableData> list =
		// this.restTemplate.exchange(LISTPAGEADDRESS, HttpMethod.POST, entity,
