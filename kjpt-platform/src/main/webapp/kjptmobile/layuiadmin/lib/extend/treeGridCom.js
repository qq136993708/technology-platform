...
layui.config({
    ...
    var $ = layui.$
        ...
        ,table = {
            ...
            ,addRow:function (tableId, index, data) {
                ...
                // alert("options.isTree===="+options.isTree);
                if(options.isTree){//处理层级
                    ...
                    if(uo) {
                    	...
                    	// alert("table.config.cols.level===="+data);
                ...
                // alert("options.isPage===="+options.isPage+"===");
                that.renderForm();
             ...
             * @returns {*}
             */
            ,addRowPcitc:function (tableId, index, data, isOpen) {
                var that=table.getClass(tableId)
                    ,options=that.config
                    ,uo = []//父级节点
                    ,treeList=table.getDataTreeList(tableId)
        ...
        //字符常量
        ,MOD_NAME = 'treeGridCom', ELEM = '.layui-table', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled', NONE = 'layui-none'
    ...
    Class.prototype.treeConvertShowName=function (o) {
        ...
        var temhtml='<div style="float: left;height: 32px;line-height: 32px;padding-left: '+
            function () {
                if(isTreeNode){
                    return '5px'
                }else{
            ...
            +function () {//图标或占位符
                var temTreeHtml='';
                var temTreeIsOpen=o[table.config.cols.isOpen]?"<img src='/kjptmobile/layuiadmin/layui/images/icon_minus.png'/>":"<img src='/kjptmobile/layuiadmin/layui/images/icon_add.png'/>";
                if(isTreeNode){//父节点
    ...
    Class.prototype.treeNodeOpen=function (o,isOpen) {
        ...
        if(o[table.config.cols.isOpen]){//打开状态
            //dbClickI.html('&#xe625;');
            dbClickI.html('<img src="/kjptmobile/layuiadmin/layui/images/icon_minus.png"/>');
            //alert('动态查询id为:'+o.id+"节点下的数据");
            // 向后台发送请求获取到该节点下的所有子节点同时查子节点的子节点
        }else{
            dbClickI.html('<img src="/kjptmobile/layuiadmin/layui/images/icon_add.png"/>');
            //dbClickI.html('&#xe623;');
        }
    };
    ...
    Class.prototype.treeIconRender=function (o,isHtml) {
        ...
        if(iconOpen){
            var temf=function () {//自定义图标
                var temhtml='<i class="layui-tree-'+o[options.idField]+'" style="display:inline-block;height:32px;width: 16px;height: 16px;background:url(';
                if(isTreeNode){//父节点
            ...
            if(isTreeNode){//父节点
                ...
                if(o[table.config.cols.icon]){
                    ...
                    temTreeHtml+='<i class="layui-icon layui-tree-'+o[options.idField]+' layui-tree-leaf"  '+iconOpen+'><img src="/kjptmobile/layuiadmin/layui/images/icon_spot.png"/></i>';
                }
            }
            if(isHtml){
                return temTreeHtml;
    ...
    Class.prototype.renderCss = function(){
        ...
        var styel='<style id="'+id+'">'
            +function () {
                var ret=".layui-tree-head{cursor: pointer;margin:0 5px;}";//树图标点击样式
    ...
    Class.prototype.events = function(){
        ...
        bodytr.unbind('click').on('click',function (e) {
            ...
            $('tbody').find("tr").eq(index).addClass('layui-table-click');
    ...
    Class.prototype.eventsinit = function(){
        ...
        that.elem.on('click','i.layui-tree-head', function(){
            ...
            var o=datas[index];
            var oId=o.id;
            table.getDataList(oId)
            //alert(that.key+"===="+o+"====="+table.config.cols.isOpen);
