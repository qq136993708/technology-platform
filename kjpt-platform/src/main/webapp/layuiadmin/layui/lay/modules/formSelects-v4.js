...
(function(layui, window, factory) {
	...
	var v = '4.0.0.0910',
		...
		FormSelects = function(options) {
			...
			this.config = {
				...
				opened: null,
				closed: null,
				filter: function(id, inputVal, val, isDisabled) {
	...
	Common.prototype.init = function(target){
		...
		$((target ? target : 'select['+NAME+']')).each(function(index, select) {
			var othis = $(select),
				...
				value = othis.find('option[selected]').toArray().map(function(option) {//获取已选中的数据
					return {
			...
			//构造渲染div
			var dinfo = _this.renderSelect(id, fs.config.placeholder, select);
			...
			$reElem += '<input class="'+HIDE_INPUT+'" value="" name="'+fs.config.formname+'" lay-verify="'+fs.config.layverify+'" lay-verType="'+fs.config.layverType+'" type="text" style="position: absolute;bottom: 0; z-index: -1;width: 100%; height: 100%; border: none; opacity: 0;"/>';
			$reElem += '<div class="'+FORM_TITLE+' '+(fs.config.disabled ? DIS : '')+'">';
			$reElem += '<div class="'+FORM_INPUT+' '+NAME+'" '+heightStyle+'>' + inputHtml.join('') + '<i class="'+SANJIAO+'"></i></div>';
			$reElem += '<div class="'+TDIV+'">';
			$reElem += '<input type="text" autocomplete="off" placeholder="'+fs.config.placeholder+'" readonly="readonly" unselectable="on" class="'+FORM_INPUT+'">';
			$reElem += '</div><div></div></div>';
			$reElem += '<dl xid="'+id+'" class="'+DL+' '+(fs.config.radio ? RADIO:'')+'">'+dinfo+'</dl></div>';
			...
			if(fs.config.isSearch){
				...
				$(document).on('input', ('div.'+PNAME+'[FS_ID="'+id+'"] .'+INPUT), function(e) {
					_this.search(id, e, fs.config.searchUrl);
				});
				if(fs.config.searchUrl){//触发第一次请求事件
					_this.triggerSearch(reElem, true);
				}
			}else{//隐藏第二个dl
		...
		if(searchUrl){
			...
			if(!ajaxConfig.beforeSearch || (ajaxConfig.beforeSearch && ajaxConfig.beforeSearch instanceof Function && ajaxConfig.beforeSearch(id, searchUrl, inputValue))){
				...
				fs.clearid = setTimeout(function() {
					...
					_this.ajax(id, searchUrl, inputValue, false, null, true);
				}, delay);
			}
		}else{
			...
			reElem.find('dl dd:not(.'+FORM_SELECT_TIPS+')').each(function(idx, item) {
				...
				if(searchFun && searchFun(id, inputValue, _this.getItem(id, _item), _item.hasClass(DISABLED)) == true){
	...
	Common.prototype.triggerSearch = function(div, isCall){
		...
		(div ? [div] : $('.'+FORM_SELECT).toArray()).forEach(function(reElem, index) {
			...
			if((id && data[id] && data[id].config.isEmpty) || isCall){
				_this.search(id, null, null, data[id].config.searchType == 0 ? reElem.find('.'+LABEL+' .'+INPUT) : reElem.find('dl .'+FORM_DL_INPUT+' .'+INPUT));
			}
		});
	}
		...
		$.ajax({
			...
			success: function(res) {
				...
				if(res[ajaxConfig.response.statusName] != ajaxConfig.response.statusCode) {
					...
					reElem.find('dd.'+FORM_NONE).removeClass(FORM_EMPTY);
					_this.renderData(id, res[ajaxConfig.response.dataName], isLinkage, linkageWidth, isSearch, isReplace);
	...
	Common.prototype.renderData = function(id, dataArr, linkage, linkageWidth, isSearch, isReplace){
		...
		if(isSearch){//如果是远程搜索, 这里需要判重
			...
			values.forEach(function(item, index) {
				if(_this.indexOf(oldVal, item) == -1){
					_this.addLabel(id, label, item);
					dl.find('dd[lay-value="'+item.value+'"]').addClass(THIS);
					oldVal.push(item);
				}
			});
	...
	Common.prototype.renderReplace = function(id, dataArr){
		...
		var html = dataArr.map(function(item)  {
			...
			return _this.createDD(id, itemVal);
		}).join('');
	...
	Common.prototype.createDD = function(id, item, clz){
		...
		var itemDD = '<dd lay-value="'+item.value+'" class="'+(item.disabled ? DISABLED : '')+' '+(clz ? clz : '')+'" '+attr+'>';
		itemDD += '<div class="xm-unselect xm-form-checkbox '+(item.disabled ? DISABLED : '')+' '+((item.children && item.children.length) ? 'x-tree-item-parent':'')+'"  style="margin-left: '+((pid.length - 1) * 30)+'px">';
	...
	Common.prototype.renderBtns = function(id, show, right){
		...
		$.each(data[id].config.btns, function(index, item)  {
			quickBtn.push(_this.createQuickBtn(item, right));
		});
		quickBtn.push('</div>');
			...
			$(select).each(function(index, item)  {
				if(item){
					if(item.type && item.type === 'optgroup') {
						...
						arr.push(_this.createDD(id, item));
					}
				}
			});
		}else{
		...
		$(document).on('click', function(e)  {
			if(!$(e.target).parents('.'+FORM_TITLE)[0]){//清空input中的值
				...
				$.each(data, function(key, fs) {
					_this.clearInput(key);
					if(!fs.values.length){
						_this.changePlaceHolder($('div[FS_ID="'+key+'"] .'+LABEL));
					}
				});
			}
			$('.'+PNAME+' .'+FORM_SELECTED).each(function(index, item)  {
				_this.changeShow($(item).find('.'+FORM_TITLE), false);
			}) ;
		});
	}
	...
	Common.prototype.one = function(target){//一次性事件绑定
		...
		$(target ? target : document).off('click', '.'+FORM_TITLE).on('click', '.'+FORM_TITLE, function(e)  {
			...
			$('dl[xid]').not(dl).each(function(index, item)  {
				_this.clearInput($(item).attr('xid'));
			});
			...
			if(othis.is('.'+SANJIAO) || othis.is('.'+INPUT+'[readonly]')){
				_this.changeShow(title, !title.parents('.'+FORM_SELECT).hasClass(FORM_SELECTED));
				return false;
			}
			...
			if(title.find('.'+INPUT+':not(readonly)')[0]){
				var input = title.find('.'+INPUT),
					...
					pos = _this.getPosition(title[0]),
					width = title.width();
				while(epos.x > pos.x){
					if($(document.elementFromPoint(epos.x, epos.y)).is(input)){
						input.focus();
						_this.changeShow(title, true);
						return false;
					}
					epos.x -= 50;
				}
		...
		$(target ? target : document).off('click', 'dl.'+DL).on('click', 'dl.'+DL, function(e)  {
			...
			if(othis.is('.'+LINKAGE) || othis.parents('.'+LINKAGE)[0]){//linkage的处理
				...
				if(!nextGroup[0] || nextGroup.find('li:not(.xm-select-linkage-hide)').length == 0){
					...
					_this.handlerLabel(id, null, isAdd, val);
				}else{
			...
			if(othis.is('i.icon-caidan')){
				...
				arr.forEach(function(item) {item.click()});
				return false;
			}
			//树状结构的选择
			...
			if(treeId){
				...
				if(othis.is('i:not(.icon-expand)')){
					_this.handlerLabel(id, dd, !dd.hasClass(THIS));
					return false;
				}
				...
				if(childrens && childrens.length && othis.is('.x-tree-icon')){
					...
					len ? (
						...
						(childrens = childrens.filter(function(index, item)  {
							if ($(item).attr('tree-id').split('-').length - 1 == treeId.split('-').length) {
								return item;
							}
						}))
					);
					...
					if(treeConfig.nextClick && treeConfig.nextClick instanceof Function){
						treeConfig.nextClick(id, _this.getItem(id, dd), function(res) {
							if(!res || !res.length){
								_this.handlerLabel(id, dd, !dd.hasClass(THIS));
							}else{
								...
								res.forEach(function(item, idx) {
									...
									ddChilds.push(_this.createDD(id, item));
			...
			if(dd.hasClass(FORM_SELECT_TIPS)){//tips的处理
				...
				var obj = data[id].config.btns.filter(function(bean) {if (bean.name == method) return bean})[0];
				obj && obj.click && obj.click instanceof Function && obj.click(id, _this);
				return false;
			}
			_this.handlerLabel(id, dd, !dd.hasClass(THIS));
			return false;
	...
	Common.prototype.initVal = function(id){
		...
		$.each(target, function(key, val) {
			...
			_vals.concat([]).forEach(function(item, index) {
				_this.addLabel(key, label, item);
				dl.find('dd[lay-value="'+item.value+'"]').addClass(THIS);
			});
			if(val.config.radio){
				_vals.length && values.push(_vals[_vals.length - 1]);
			}
			_this.commonHandler(key, label);
		});
	}
	...
	Common.prototype.addLabel = function(id, div, val){
		if(!val) return ;
		var tips = 'fsw="'+NAME+'"',
				$label = $('<span '+tips+' value="'+val.value+'"><font '+tips+'>'+val.name+'</font></span>'),
				$close = $('<i '+tips+' class="xm-iconfont icon-close"></i>');
	...
	Common.prototype.selectAll = function(id, isOn, skipDis){
		...
		dl.find('dd[lay-value]:not(.'+FORM_SELECT_TIPS+'):not(.'+THIS+')'+(skipDis ? ':not(.'+DISABLED+')' :'')).each(function(index, item)  {
			item = $(item);
			var val = _this.getItem(id, item);
			_this.handlerLabel(id, dl.find('dd[lay-value="'+val.value+'"]'), true, val, !isOn);
		});
	}
	...
	Common.prototype.removeAll = function(id, isOn, skipDis){
		...
		data[id].values.concat([]).forEach(function(item, index) {
			if(skipDis && dl.find('dd[lay-value="'+item.value+'"]').hasClass(DISABLED)){
				...
				_this.handlerLabel(id, dl.find('dd[lay-value="'+item.value+'"]'), false, item, !isOn);
			}
		});
	}
	...
	Common.prototype.reverse = function(id, isOn, skipDis){
		...
		dl.find('dd[lay-value]:not(.'+FORM_SELECT_TIPS+')'+(skipDis ? ':not(.'+DISABLED+')' :'')).each(function(index, item)  {
			item = $(item);
			var val = _this.getItem(id, item);
			_this.handlerLabel(id, dl.find('dd[lay-value="'+val.value+'"]'), !item.hasClass(THIS), val, !isOn);
		});
	}
	...
	Select4.prototype.config = function(id, config, isJson){
		...
		if(config && typeof config== 'object'){
			...
			id ? (
				ajaxs[id] = $.extend(true, {}, ajaxs[id] || ajax, config), !common.check(id) && _this.render(id),
	...
	Select4.prototype.render = function(id, options){
		...
		($('select['+NAME+'="'+id+'"]')[0] ? $('select['+NAME+'="'+id+'"]') : $('select['+NAME+']')).each(function(index, select) {
			...
			setTimeout(function() {common.setHidnVal(sid, $('select[xm-select="'+sid+'"] + div.'+PNAME+' .'+LABEL))}, 10);
		});
		return this;
	}
	...
	Select4.prototype.btns = function(id, btns, config){
		...
		$.each(target, function(key, val) {
			...
			if(btns.length){
				...
				var html = pcInput.attr('placeholder') || pcInput.attr('back');
				dd.html(html);
				dd.removeAttr('style');
			}
		});
