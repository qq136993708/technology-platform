layui.use(['form', 'jquery', 'table', 'layer', 'laydate'], function(){
	...
	function getItemData(data) {
		var httpUrl = '/researchPlatformPatent-api/newInit/' + data.platformId;
		if (data.id) {
			httpUrl = '/researchPlatformPatent-api/load/' + data.id
		}
	...
	form.on('submit(InputSubmit)', function(data) {
		...
		httpModule({
			type: 'POST',
			url: '/researchPlatformPatent-api/save',
			data: data.field,
			success: function(res) {
				if (res.code === '0') {
					setDialogData(res);
	...
	$('#projectSubmit').on('click', function(e) {
      ...
      var tableCheckedData = table.checkStatus('unInputTable').data;
			...
			httpModule({
				type: 'POST',
				url: '/researchPlatformPatent-api/batchSave',
