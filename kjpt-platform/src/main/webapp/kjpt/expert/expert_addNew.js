layui.config({
    ...
    /*职称*/
    createElement("ROOT_KJPT_ZJZC", "title", "option", "title")
    /*职务*/
    ...
    httpModule({
        ...
        success: function (relData) {
            ...
            //     })
            // })
            formSelects.data('technicalField', 'local', {
    ...
    httpModule({
        ...
        async: false,
        success: function (relData) {
            formSelects.data('belongUnit', 'local', {
                arr: relData.children
            });
    ...
    //输入框的值改变时触发
    $('input[name="idCardNo"]').on("input", function (e) {
        $(this).attr('lay-verify', 'identity')
    });
    /*保存*/
    form.on('submit(newSubmit)', function (data) {
        if (data.field.sex == undefined) {
            layer.msg("性别必为填项不能为空！", {
                icon: 2
            });
            return false
        }
        if (data.field.belongUnit == '') {
            layer.msg("所在单位必为选项不能为空！", {
                icon: 2
            });
            return false
        }
