package com.pcitc.service.expert;

import com.pcitc.base.common.LayuiTableData;
import com.pcitc.base.common.LayuiTableParam;
import com.pcitc.base.common.TreeNode;
import com.pcitc.base.expert.TechFamilyType;
import com.pcitc.base.expert.TechFamilyTypeExample;
import org.apache.ibatis.annotations.Param;

import java.util.List;

/**
 * <p>接口类</p>
 * <p>Table: tech_family_type - 技术族-分类</p>
 *
 * @since 2018-12-29 11:34:35
 */
public interface TechFamilyTypeService {

    /**
     * 判空-信息
     *
     * @param record
     * @return
     * @throws Exception
     */
    List<TechFamilyType> findTechFamilyTypeList(TechFamilyType record) throws Exception;

    /**
     * 删除
     *
     * @param record
     * @return
     * @throws Exception
     */
    int updateOrInsertTechFamilyType(TechFamilyType record) throws Exception;

    /**
     * 根据id删除
     *
     * @param id
     * @return
     * @throws Exception
     */
    int deleteTechFamilyTypeById(String id) throws Exception;

    /**
     * 根据id查询对象
     *
     * @param id
     * @return
     * @throws Exception
     */
    TechFamilyType getTechFamilyTypeInfo(String id) throws Exception;

    /**
     * 查询总条数
     *
     * @param example
     * @return
     */
    long countByExample(TechFamilyTypeExample example);

    /**
     * 根据example条件删除
     *
     * @param example
     * @return
     */
    int deleteByExample(TechFamilyTypeExample example);

    /**
     * 根据主键删除
     *
     * @param id
     * @return
     */
    int deleteByPrimaryKey(String id);

    /**
     * 插入信息
     *
     * @param record
     * @return
     */
    int insert(TechFamilyType record);

    /**
     * 插入信息,返回对象
     *
     * @param record
     * @return
     */
    TechFamilyType insertObject(TechFamilyType record);

    /**
     * 判空-插入信息
     *
     * @param record
     * @return
     */
    int insertSelective(TechFamilyType record);

    /**
     * 根据example条件查询
     *
     * @param example
     * @return
     */
    List<TechFamilyType> selectByExample(TechFamilyTypeExample example);

    /**
     * 根据主键查询
     *
     * @param id
     * @return
     */
    TechFamilyType selectByPrimaryKey(String id);

    /**
     * 根据example条件-判空更新
     *
     * @param record
     * @param example
     * @return
     */
    int updateByExampleSelective(@Param("record") TechFamilyType record, @Param("example") TechFamilyTypeExample example);

    /**
     * 根据example条件,更新所有
     *
     * @param record
     * @param example
     * @return
     */
    int updateByExample(@Param("record") TechFamilyType record, @Param("example") TechFamilyTypeExample example);

    /**
     * 根据主键判空-更新
     *
     * @param record
     * @return
     */
    int updateByPrimaryKeySelective(TechFamilyType record);

    /**
     * 根据主键更新所有
     *
     * @param record
     * @return
     */
    int updateByPrimaryKey(TechFamilyType record);

    /**
     * 删除菜单
     *
     * @param id
     * @return
     */
    Integer deleteTechFamilyType(java.io.Serializable id);

    /**
     * @param param 分页查询
     * @return
     */
    LayuiTableData findTechFamilyTypeByPage(LayuiTableParam param);

    /**
     * 真删除
     *
     * @param id
     * @return
     */
    Integer deleteTechFamilyTypeReal(String id);

    /**
     * 查询树
     *
     * @return
     */
    List<TreeNode> selectObjectByTree();
}