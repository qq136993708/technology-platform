...
//        c.andStatusEqualTo("1");
		...
		/*
		 * List<ZjkChoice> zjkChoices = (List<ZjkChoice>) data.getData();
		 * OutProjectInfoExample ex = new OutProjectInfoExample(); List<String> xmids =
		 * zjkChoices.stream().map(e -> e.getXmId()).collect(Collectors.toList()); if
		 * (xmids == null || xmids.size() == 0) { xmids = new ArrayList<>(); }
		 * ex.createCriteria().andXmidIn(xmids); List<OutProjectInfo> outProjectInfos =
		 * systemRemoteClient.selectByExample(ex);
		 * 
		 * if (outProjectInfos!=null&&outProjectInfos.size()>0){ for (int i = 0; i <
		 * outProjectInfos.size(); i++) { if
		 * (outProjectInfos.get(i).getFzdw()==null||"".equals(outProjectInfos.get(i).
		 * getFzdw())){ outProjectInfos.get(i).setFzdw("无"); } } Map<String,String> map
		 * = outProjectInfos.stream().collect(Collectors.toMap(OutProjectInfo::getXmid,
		 * OutProjectInfo::getFzdw,(e1,e2)->e1)); for (int i = 0, j = zjkChoices.size();
		 * i < j; i++) {
